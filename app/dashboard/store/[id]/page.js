...
export default function ManageStore() {
  ...
  const fetchStore = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/stores/${params.id}`,
      )
      if (response.ok) {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/products/store/${params.id}`,
      )
      if (response.ok) {
        const data = await response.json()
        setProducts(data)
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start space-x-6">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-500">
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${store.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
            ...
            <div className="flex space-x-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium ${store.isActive ? "bg-red-100 text-red-800 hover:bg-red-200" : "bg-green-100 text-green-800 hover:bg-green-200"}`}
              >
                {store.isActive ? "Desactivar" : "Activar"}
              </button>
              <Link href={`/dashboard/store/${store._id}/edit`} className="btn-secondary">
                Editar
              </Link>
            </div>
          </div>
